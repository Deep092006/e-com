  ...
  return (
    <section className="space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="text-[24px] font-bold tracking-[-0.18px] text-[#222222]">{title}</h2>
        <div className="flex items-center gap-2">
          <button className="swipe-btn" onClick={() => scrollRow(-1)} type="button">
            <ChevronLeft size={16} />
          </button>
          <button className="swipe-btn" onClick={() => scrollRow(1)} type="button">
  ...
  return (
    <section className="space-y-8">
      <div className="no-scrollbar flex gap-2 overflow-x-auto pb-1">
        {types.map((type) => (
          <button
            className={`rounded-full border px-4 py-2 text-[13px] font-semibold ${selectedType === type ? "border-[#222222] bg-[#222222] text-white" : "border-[#ebebeb] text-[#6a6a6a] hover:text-[#222222]"}`}
            key={type}
      ...
      {loading && <div className="surface-card p-6 text-[14px] text-[#6a6a6a]">Loading products...</div>}
      {!loading && error && <div className="surface-card p-6 text-[14px] text-[#c13515]">{error}</div>}
      {!loading && !error && filteredProducts.length === 0 && <div className="surface-card p-6 text-[14px] text-[#6a6a6a]">No products found.</div>}
      ...
      {!loading && !error && filteredProducts.length > 0 && (
        <>
          ...
          <section className="space-y-3">
            <h2 className="text-[24px] font-bold tracking-[-0.18px] text-[#222222]">More products</h2>
